...
exports.CrackInterpreter = void 0;
exports.main = main;
...
class AIFixProvider {
    ...
    static async fixCodeWithAI(code, error) {
...
AIFixProvider.API_URL = 'http://localhost:3700';
class ErrorHandler {
    static async showCriticalError(message, line, suggestion, code, filename) {
        console.log(`
...
`);
    ...
    static showWarning(message, line, token) {
        console.log(`⚠️  ПРЕДУПРЕЖДЕНИЕ (строка ${line}): ${message} '${token}'`);
    }
    static showUnknownToken(token, line) {
...
`);
            ...
            while (current < trimmedLine.length) {
                ...
                if (char === '"') {
                    ...
                    if (current >= trimmedLine.length) {
                        ErrorHandler.showCriticalError('Незакрытая строка', startLine, 'Добавьте закрывающую кавычку "');
        ...
        while (current < tokens.length && tokens[current].type !== TokenType.EOF) {
            ...
            if (token.type === TokenType.KEYWORD && token.value === 'conlog') {
                ...
                if (!nextToken || nextToken.type === TokenType.EOF) {
                    ...
                    await ErrorHandler.showCriticalError('Команда conlog требует аргумент', token.line, 'Используйте: conlog "текст" или conlog (переменная)', originalCode, this.currentFilename);
                    return ast;
                }
                if (nextToken.type === TokenType.STRING) {
                    ...
                    if (!varToken || varToken.type !== TokenType.IDENTIFIER) {
                        ErrorHandler.showCriticalError('Неправильный синтаксис conlog', token.line, 'Используйте: conlog (имя_переменной)');
                        current++;
                        continue;
                    }
                    current++;
                    ...
                    if (!closeToken || closeToken.value !== ')') {
                        ErrorHandler.showCriticalError('Незакрытая скобка в conlog', token.line, 'Добавьте закрывающую скобку )');
                    }
                    else {
                        if (!this.variables.has(varToken.value)) {
                            ErrorHandler.showWarning('Переменная не определена', varToken.line, varToken.value);
                        }
                        ast.push({
                            type: 'FunctionCall',
                            name: 'conlog',
                    ...
                    const originalCode = fs.readFileSync(this.currentFilename, 'utf-8');
                    await ErrorHandler.showCriticalError('Неправильный аргумент для conlog', nextToken.line, 'Используйте: conlog "строка" или conlog (переменная)', originalCode, this.currentFilename);
                    return ast;
                }
            }
                ...
                if (nextToken && nextToken.type === TokenType.OPERATOR && nextToken.value === '=') {
                    ...
                    if (!valueToken) {
                        ErrorHandler.showCriticalError('Неполное присваивание переменной', token.line, 'Добавьте значение после знака =');
                        break;
                    }
                    ...
                    ErrorHandler.showWarning('Неопознанная конструкция', token.line, token.value);
                    current++;
                }
            }
                ...
                if (!colonToken || colonToken.value !== ':') {
                    ErrorHandler.showCriticalError('Неправильный синтаксис codego', token.line, 'Используйте: codego:(имя_переменной)');
                    current++;
                    continue;
                }
                current++;
                ...
                if (!openToken || openToken.value !== '(') {
                    ErrorHandler.showCriticalError('Отсутствует скобка в codego', token.line, 'Используйте: codego:(имя_переменной)');
                    current++;
                    continue;
                }
                current++;
                const varToken = tokens[current];
                if (!varToken || varToken.type !== TokenType.IDENTIFIER) {
                    ErrorHandler.showCriticalError('Неправильное имя переменной в codego', token.line, 'Укажите имя переменной: codego:(mycode)');
                    current++;
                    continue;
                }
                current++;
                const closeToken = tokens[current];
                if (!closeToken || closeToken.value !== ')') {
                    ErrorHandler.showCriticalError('Незакрытая скобка в codego', token.line, 'Добавьте закрывающую скобку )');
                }
                else {
                    if (!this.variables.has(varToken.value)) {
                        ErrorHandler.showWarning('Переменная для codego не определена', varToken.line, varToken.value);
                    }
                    ast.push({
                        type: 'CodeExecution',
                ...
                if (!moduleToken || moduleToken.type !== TokenType.IDENTIFIER) {
                    ErrorHandler.showCriticalError('Команда imp требует имя модуля', token.line, 'Используйте: imp имя_модуля');
                    current++;
                    continue;
                }
                ast.push({
                ...
                ErrorHandler.showWarning('Неопознанный токен', token.line, token.value);
                current++;
            }
        }
        return ast;
    }
    async parseIfStatement(tokens, start) {
        let current = start + 1;
        if (!tokens[current] || tokens[current].value !== '(') {
            ErrorHandler.showCriticalError('Ожидается скобка после if', tokens[start].line, 'Используйте: if (условие):');
        ...
        if (!tokens[current] || tokens[current].value !== ')') {
            ErrorHandler.showCriticalError('Ожидается закрывающая скобка', tokens[start].line, 'Добавьте ) после условия');
            return { node: null, newPosition: current };
        }
        current++;
        if (!tokens[current] || tokens[current].value !== ':') {
            ErrorHandler.showCriticalError('Ожидается : после условия', tokens[start].line, 'Используйте: if (условие):');
            return { node: null, newPosition: current };
        }
        current++;
        if (!tokens[current] || tokens[current].type !== TokenType.INDENT) {
            ErrorHandler.showCriticalError('Ожидается отступ после :', tokens[start].line, 'Сделайте отступ для блока кода');
            return { node: null, newPosition: current };
        }
        current++;
        ...
        if (tokens[current] && tokens[current].type === TokenType.KEYWORD && tokens[current].value === 'else') {
            current++;
            if (!tokens[current] || tokens[current].value !== ':') {
                ErrorHandler.showCriticalError('Ожидается : после else', tokens[current - 1].line, 'Используйте: else:');
                return { node: null, newPosition: current };
            }
            current++;
            if (!tokens[current] || tokens[current].type !== TokenType.INDENT) {
                ErrorHandler.showCriticalError('Ожидается отступ после else:', tokens[current - 1].line, 'Сделайте отступ для блока else');
                return { node: null, newPosition: current };
            }
            current++;
    ...
    async parseWhileStatement(tokens, start) {
        let current = start + 1;
        if (!tokens[current] || tokens[current].value !== '(') {
            ErrorHandler.showCriticalError('Ожидается скобка после while', tokens[start].line, 'Используйте: while (условие):');
            return { node: null, newPosition: current };
        }
        current++;
        const condition = await this.parseCondition(tokens, current);
        ...
        current++;
        if (!tokens[current] || tokens[current].value !== ':') {
            ErrorHandler.showCriticalError('Ожидается : после условия', tokens[start].line, 'Используйте: while (условие):');
            return { node: null, newPosition: current };
        }
        current++;
        if (!tokens[current] || tokens[current].type !== TokenType.INDENT) {
    ...
    async parseForStatement(tokens, start) {
        let current = start + 1;
        if (!tokens[current] || tokens[current].value !== '(') {
            ErrorHandler.showCriticalError('Ожидается скобка после for', tokens[start].line, 'Используйте: for (i = 0; i < 10; i = i + 1) { код }');
            return { node: null, newPosition: current };
        }
        current++;
        const initToken = tokens[current];
        if (!initToken || initToken.type !== TokenType.IDENTIFIER) {
            ErrorHandler.showCriticalError('Ожидается переменная в for', tokens[start].line, 'Используйте: for (переменная = начало; ...');
            return { node: null, newPosition: current };
        }
        current++;
        if (!tokens[current] || tokens[current].value !== '=') {
            ErrorHandler.showCriticalError('Ожидается = в for', tokens[start].line, 'Используйте: for (i = 0; ...');
            return { node: null, newPosition: current };
        }
        current++;
        const startValueToken = tokens[current];
        if (!startValueToken || startValueToken.type !== TokenType.NUMBER) {
            ErrorHandler.showCriticalError('Ожидается число в for', tokens[start].line, 'Используйте: for (i = 0; ...');
            return { node: null, newPosition: current };
        }
        current++;
        if (!tokens[current] || tokens[current].value !== ';') {
            ErrorHandler.showCriticalError('Ожидается ; в for', tokens[start].line, 'Используйте: for (i = 0; условие; ...');
            return { node: null, newPosition: current };
        }
        current++;
        const condition = await this.parseCondition(tokens, current);
        current = condition.newPosition;
        if (!tokens[current] || tokens[current].value !== ';') {
            ErrorHandler.showCriticalError('Ожидается ; после условия в for', tokens[start].line, 'Используйте: for (...; условие; инкремент)');
            return { node: null, newPosition: current };
        }
        current++;
        const incrementVar = tokens[current];
        if (!incrementVar || incrementVar.type !== TokenType.IDENTIFIER) {
            ErrorHandler.showCriticalError('Ожидается переменная в инкременте for', tokens[start].line, 'Используйте: for (...; ...; i = i + 1)');
            return { node: null, newPosition: current };
        }
        current++;
        if (!tokens[current] || tokens[current].value !== '=') {
            ErrorHandler.showCriticalError('Ожидается = в инкременте for', tokens[start].line, 'Используйте: for (...; ...; i = i + 1)');
            return { node: null, newPosition: current };
        }
        current++;
        let incrementExpr = '';
        ...
        if (!tokens[current] || tokens[current].value !== ')') {
            ErrorHandler.showCriticalError('Ожидается закрывающая скобка в for', tokens[start].line, 'Добавьте ) в конце for');
            return { node: null, newPosition: current };
        }
        current++;
        if (!tokens[current] || tokens[current].value !== ':') {
            ErrorHandler.showCriticalError('Ожидается : после for', tokens[start].line, 'Используйте: for (...):');
            return { node: null, newPosition: current };
        }
        current++;
        if (!tokens[current] || tokens[current].type !== TokenType.INDENT) {
    ...
    async parseCondition(tokens, start) {
        let current = start;
        let leftSide = '';
    ...
    async parseIndentedBlock(tokens, start) {
        const body = [];
        let current = start;
...
exports.CrackInterpreter = CrackInterpreter;
async function main() {
